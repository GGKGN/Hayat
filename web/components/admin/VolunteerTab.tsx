...
export default function VolunteerTab({ questions, applications, isOpen }: { questions: any[], applications: any[], isOpen: boolean }) {
    ...
    const prepareExportData = () => {
        return applications.map(app => {
            ...
            const answersText = answers?.map((a: any) => `S: ${a.question}\nC: ${a.answer}`).join('\n\n') || ''

            return {
                "Ad Soyad": app.user.name,
                "E-posta": app.user.email,
                "Telefon": app.user.phoneNumber || '-',
                "Takım": app.team.name,
    ...
    const handleExportExcel = () => {
        ...
        exportToExcel(`Gonullu_Basvurulari_${new Date().toLocaleDateString('tr-TR')}`, data)
    }

    const handleExportPDF = () => {
        const data = prepareExportData()
        const columns = ["Ad Soyad", "E-posta", "Telefon", "Takım", "Durum", "Başvuru Tarihi", "Yanıtlar"]
        ...
        exportToPDF(`Gonullu_Basvurulari_${new Date().toLocaleDateString('tr-TR')}`, columns, rows, 'Gönüllü Başvuruları Listesi')
    }
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                {subTab === "APPLICATIONS" && (
                    <div className="flex gap-2">
                        <button onClick={handleExportExcel} className="flex items-center gap-2 px-4 py-2 bg-green-50 text-green-700 rounded-lg hover:bg-green-100 font-bold text-sm transition-colors border border-green-200">
                            <FileSpreadsheet className="w-4 h-4" /> Excel İndir
                        </button>
                        <button onClick={handleExportPDF} className="flex items-center gap-2 px-4 py-2 bg-red-50 text-red-700 rounded-lg hover:bg-red-100 font-bold text-sm transition-colors border border-red-200">
